    ...
    <div class="w-full max-w-2xl">
        <div class="bg-white rounded-3xl shadow-2xl p-10 border border-purple-100">
            <div class="mb-8 flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-purple-800 tracking-tight">Edit Building</h1>
                    <p class="text-gray-500 text-sm mt-1">Update building information</p>
                </div>
            </div>
            ...
            <form action="{{ url('admin/buildings/'.$building->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-gray-900 mb-2">Building Name *</label>
                    <input type="text" name="name" value="{{ $building->name }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-900 mb-2">Address</label>
                    <textarea name="address" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">{{ $building->address }}</textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-900 mb-2">Total Floors</label>
                        <input type="number" name="total_floors" value="{{ $building->total_floors }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-900 mb-2">Total Flats</label>
                        <input type="number" name="total_flats" value="{{ $building->total_flats }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                </div>

                <div class="flex space-x-4 pt-6 border-t">
                    <button type="submit" class="flex-1 bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-lg transition">
                        <i class="fas fa-save mr-2"></i>Update Building
                    </button>
