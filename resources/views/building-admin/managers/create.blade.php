        ...
        <form method="POST" action="{{ route('building-admin.managers.store') }}" class="space-y-4">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" required class="w-full px-4 py-3 rounded-lg border border-border dark:border-gray-700 bg-white dark:bg-surface-dark text-text-main dark:text-white placeholder-text-sub dark:placeholder-text-sub focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Enter phone number">
                    @error('phone')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <button type="submit" class="w-full mt-6 bg-primary text-white rounded-lg shadow-sm px-4 py-3 text-base font-bold hover:bg-primary/90 transition">Create Manager</button>
        </form>
    </main>
</div>
@endsection