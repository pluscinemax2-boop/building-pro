...
<div class="min-h-screen bg-background-light dark:bg-background-dark">
    ...
    <header class="sticky top-0 z-30 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-5 py-4">
        <div class="flex items-center justify-between max-w-2xl mx-auto">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Create New Poll</h1>
    ...
    <section class="px-5 py-8 max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('building-admin.polls.store') }}" class="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-6">
            @csrf

            <!-- Question -->
            <div>
                <label class="block text-sm font-semibold text-slate-900 dark:text-white mb-2">Poll Question *</label>
                <input type="text" name="question" value="{{ old('question') }}" placeholder="E.g., What should be the color of the lobby?"
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Provide more context or details about this poll..."
                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/30 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-red-600 dark:text-red-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
