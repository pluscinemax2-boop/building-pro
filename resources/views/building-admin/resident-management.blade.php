...
<div class="min-h-screen relative pb-24 bg-background-light dark:bg-background-dark">
    ...
    <main class="w-full max-w-md mx-auto flex flex-col gap-4">
        <!-- Search (functional) & Filter Chips Section -->
        <section class="px-4 pt-2 pb-0">
            <div class="flex flex-col gap-2">
                <form method="GET" action="" class="flex gap-2">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search residents..." class="flex-1 rounded-full border border-border dark:border-gray-700 bg-white dark:bg-surface-dark px-4 py-2 text-sm text-text-main dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/20 transition" />
                    <button type="submit" class="rounded-full bg-primary text-white px-4 py-2 font-medium hover:bg-primary/90 transition-colors">Search</button>
                </form>
    ...
    <!-- Footer Add Resident Button -->
    <div class="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-surface-dark border-t border-border dark:border-gray-800 pb-4 pt-3 flex justify-center">
        <a href="{{ route('building-admin.residents.create') }}" class="w-full max-w-md mx-auto flex items-center justify-center gap-2 px-6 py-3 rounded-full bg-primary text-white font-semibold text-base shadow-card hover:bg-primary/90 transition">
