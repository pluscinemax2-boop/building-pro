...
<head>
    ...
    <title>Edit Resident - Building Admin</title>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg p-8">
            <div class="mb-8">
                ...
                <p class="text-gray-600 text-sm mt-2">Update resident details</p>
            </div>
            @if($errors->any())
            ...
            @endif
            <form method="POST" action="/building-admin/residents/{{ $resident->id }}">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-900 mb-2">Name *</label>
                    <input type="text" name="name" value="{{ old('name', $resident->name) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-900 mb-2">Email *</label>
                    <input type="email" name="email" value="{{ old('email', $resident->email) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-900 mb-2">Phone *</label>
                    <input type="text" name="phone" value="{{ old('phone', $resident->phone) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-900 mb-2">Flat *</label>
                    <select name="flat_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="{{ $flat->id }}" {{ old('flat_id', $resident->flat_id) == $flat->id ? 'selected' : '' }}>{{ $flat->flat_number }}</option>
                        @endforeach
                    </select>
                </div>
