...
@section('title', 'Notices - Manager')

@section('content')
<div class="p-4">
    <h3 class="text-lg font-bold mb-4">Notices</h3>
    ...
    <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="flex flex-col gap-3 rounded-xl p-4 bg-white dark:bg-gray-800 shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-none border border-transparent dark:border-gray-800">
            ...
            <div>
                <p class="text-[#111418] dark:text-white text-3xl font-bold leading-tight">{{ $notices->count() > 0 ? $notices->count() : count($notices) }}</p>
                <p class="text-[#617589] dark:text-gray-400 text-sm font-medium mt-1">Total Notices</p>
            </div>
        </div>
        <div class="flex flex-col gap-3 rounded-xl p-4 bg-white dark:bg-gray-800 shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-none border border-transparent dark:border-gray-800">
            <div class="flex items-center justify-between">
            ...
            <div>
                <p class="text-[#111418] dark:text-white text-3xl font-bold leading-tight">{{ $notices->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
        ...
        <div class="flex items-center justify-between gap-4 rounded-xl bg-white dark:bg-gray-800 p-3 shadow-sm border border-gray-100 dark:border-gray-700">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col justify-center">
                    <p class="text-[#111418] dark:text-white text-sm font-semibold leading-normal line-clamp-1">{{ $notice->title ?? 'Notice Title' }}</p>
                    <p class="text-[#617589] dark:text-gray-400 text-xs font-normal leading-normal">{{ $notice->category ?? 'General' }} • {{ $notice->created_at->format('d M Y') ?? '18 Dec 2023' }}</p>
                </div>
            </div>
